...
def initialize_gen_trt_engine_experiment(cfg):
    ...
    # Optional dynamic H/W bounds from spec; forwarded only when set.
    for k in ("min_height", "opt_height", "max_height", "min_width", "opt_width", "max_width"):
        try:
            v = cfg.gen_trt_engine.get(k, None)
        except Exception:
            v = None
        if v is not None:
            engine_builder_kwargs[k] = v
